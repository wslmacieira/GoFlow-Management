...
export default function EmbarcadorCadastrar() {
  ...
  async function handleSubmit() {
    ...
    const response = await api.post("/embarcadores", data);

    if (response.status === 201) {
      window.location.href = "/admin/embarcadores"
    } else {
      alert("Erro ao cadastrar embarcador!")
    }
    ...
    <div className={classes.root}>
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          <Link to="/admin/embarcadores" style={{ textDecoration: 'none' }} >
            <Button variant="contained" style={{ marginBottom: 15 }}>
              Voltar
              </Button>
          </Link>
          <Paper className={classes.paper}>
            <h2>Cadastro de Embarcadores</h2>
